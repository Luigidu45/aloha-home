...
El [manifiesto del entorno](/docs/development/asistencia_domestica_modular/historico_alohamini2/fase_0_entorno.json) registra versiones instaladas, plataforma, hashes de `pyproject.toml`, `uv.lock` y el registro resultante. Las dependencias y el lockfile permanecieron iguales. El manifiesto identifica el commit base más los cambios de esta fase; no pretende identificar un nuevo commit ya publicado.
