...
La escena principal (`dimos/robot/alohamini2/assets/household_navigation.xml`, ruta histórica) tiene dos zonas separadas por una pared con un paso de 2,4 m: sala al oeste y dormitorio al este, dentro de un recinto de 8 × 6 m. Hay mesa de recogida, botella, control remoto en el suelo, mesa de entrega con región coloreada y una cama. La variante bloqueada (`dimos/robot/alohamini2/assets/household_navigation_blocked.xml`, ruta histórica) cierra el paso. Son primitivas sintéticas y objetos fijos; no se han simulado agarres ni movimientos del objeto.
...
El blueprint household-navigation-sim (`dimos/robot/alohamini2/blueprints/household_navigation_sim.py`, ruta histórica) compone seis módulos:
...
Se amplió la función pública de composición AlohaMini2 (`dimos/robot/alohamini2/blueprints/alohamini2_nav_sim.py`, ruta histórica) para elegir escena y posición inicial conservando el perfil ligero. Los blueprints previos mantienen sus configuraciones. El registro se regeneró con la prueba oficial; se añadieron el blueprint y el módulo espacial.
...
Los tiempos usan el reloj Unix del host. El tiempo de física de MuJoCo puede avanzar más lentamente que el reloj del host; estos ensayos no miden rendimiento del hardware futuro. La publicación RGB (`dimos/robot/alohamini2/sim_module.py`, ruta histórica) ahora conserva el instante de captura del frame en lugar de renovarlo al publicar. La [nube combinada](/dimos/simulation/engines/mujoco_sim_module.py) usa el instante de la captura más antigua que contribuye, evitando que un raycast nuevo rejuvenezca puntos viejos. No es una nube con tiempos por punto.
