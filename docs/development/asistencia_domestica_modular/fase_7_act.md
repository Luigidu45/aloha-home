...
El texto del BOM antiguo describe brazos derivados del SO100 y seguidores de 12 V, pero el mismo repositorio AlohaMini1 ya incluye STL de piezas `*_SO101.stl`, tanto comunes como de la pinza seguidora. La decisión del usuario fija los seguidores de 7,4 V; no altera el BOM de base/elevador. **7,4 V identifica la variante del servo**, no una autorización para reutilizar la alimentación de 12 V de la base ni una especificación medida de la fuente final.

El simulador usa el URDF y los meshes originales de AlohaMini1, con los brazos bloqueados por límites cero. Aún no se ha comprobado si sus meshes compuestos `right_link*`/`left_link*` corresponden exactamente a las piezas SO101 ya presentes en `hardware/arms/stl`, ni se ha medido su montaje. Por tanto, F8 debe verificar esa correspondencia y corregir solo las diferencias que encuentre; no se puede afirmar que haga falta sustituir toda la geometría. También falta verificar qué eje del URDF representa la pinza SO101, incluidos `right_joint6` y `left_joint6`. La [ficha actualizada](/docs/development/asistencia_domestica_modular/fase_1_hardware.md) y la [plantilla física](/dimos/experimental/household_assistant/configs/act_hardware_pending.json) separan hechos, propuesta de interfaz y mediciones pendientes.
